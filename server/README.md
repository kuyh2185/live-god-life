...
5. 이제 채팅 화면에서 자유롭게 문장을 입력하면, 규칙 기반 해석 대신 이
   ...
   기능은 계속 동작해요).
6. 같은 서버 주소로 "오늘 좀 피곤하시군요" 화면의 "다르게 조정하기"도 함께
   켜져요 — "집 정리 때문에 바빠요"처럼 앱이 미리 정해둔 축소판이 없는
   이유를 자유 문장으로 설명하면, 이 서버(`/api/adjust-burden`)가 오늘 남은
   일정 중 뭘 줄이고 뭘 건너뛸지 직접 판단해서 돌려줘요. 서버 주소가 없으면
   이 화면은 예전처럼 일정별 수동 선택으로 안내해요.
